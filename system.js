...
(function () {
    ...
    var basicBot = {
        ...
        settings: {
            ...
            lockskipPosition: 3,
            lockskipReasons: [
                ["theme", "Tento song nesedí k téme komunity. "],
                ["op", "Tento song je na OverPlay liste. "],
                ["history", "Tento song je v histórii. "],
                ["mix", "Tvoj song, porušuje pravidlá. "],
                ["sound", "Tento song obsahuje zlý zvuk. "],
                ["nsfw", "Tento song obsahuje kontext NSFW! "],
                ["unavailable", "Tento song je dostupný len pre niekoho. "]
            ],
            ball: [
            "Vesmír poukazuje že áno.",
            "Ano.",
            "Odpoveď je hmlistá, skús neskôr.",
            "Bezpochyby.",
            "Moje zdroje hovoria nie.",
            "Vidím to na áno.",
            "Urazil si ma..",
            "Určite nie!",
            "Je to len vo hviezdach.",
            "Ano.",
            "Nie.",
            "Bez debaty.",
            "Ano..počkaj radšej nie.",
            "To je isté.",
            "Neviem ti takto z hlavy povedať.",
            ...
            "Ano, zamozrejme.",
            "Hovorím nie.",
            "Moje rozhodnutie dospelo k áno.",
            "To je nemožné.",
            "Sranda, že?",
            "Áno, nie si sexy.",
            "Pfu, dám si pauzu.",
            "Daj si radšej cookie!",
            "To si mám vycucať z palca?",
            "Si kráľ.",
            "Radšej neodpovedám.",
            "Ano, si boží.",
            "Toto nie je spoveď.."
        ...
        chatUtilities: {
            chatFilter: function (chat) {
                ...
                var rlJoinChat = basicBot.chat.hram;
        ...
        startup: function () {
            ...
            API.chatLog('Avatary nastavené na> ' + basicBot.settings.startupCap);
            API.chatLog('Volume nastavené na> ' + basicBot.settings.startupVolume);
        ...
        commands: {
            ...
            cookieCommand: {
                ...
                rank: 'user',
                type: 'startsWith',
                cookies: ['dostal/a si čokoládovú :cookie:.',
                    'dostal/a si starú, skazenú :cookie:.Nejedz to!',
                    'dostal/a si vanilkovú :cookie:. Dobrú chuť.',
                    'dostal/a si karamelovú :cookie:.',
                    'dostal/a si svoju šťastnú :cookie:. Nechaj si ju na horšie časy.',
                    'dostal/a si :cookie: na ktorej je lístok "Si môj život." :yellow_heart:.',
                    'dostal/a si :cookie: na ktorej je lístok "Kúp mi Snickers! :v:.',
                    'dostal/a si :cookie: ktorá je pečená s láskou.',
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (basicBot.room.roulette.rouletteStatus && basicBot.room.roulette.participants.indexOf(chat.uid) < 0) {
                            ...
                            API.sendChat(subChat(basicBot.chat.hram, {name: chat.un}));
                        }
                    }
                }
            },
            ...
            offCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        storeToStorage();
                        API.sendChat(basicBot.chat.off);
                        basicBot.disconnectAPI();
                        setTimeout(function () {
                            kill();
                        }, 1000);
            ...
            premiumCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat(subChat(basicBot.chat.premium, {name: chat.un}));
                    }
                }
            },
	    ...
	    giftCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var space = msg.indexOf(' ');
                        if (space === -1) {
                            API.sendChat(basicBot.chat.givegift);
                            return false;
                        }
                        else {
                            var name = msg.substring(space + 2);
                            var user = basicBot.userUtilities.lookupUserName(name);
                            if (user === false || !user.inRoom) {
                                return API.sendChat(subChat(basicBot.chat.nogift, {name: name}));
                            }
                            else if (user.username === chat.un) {
